/// The Bech32 compressed public key.
const COMPRESSED_PUBKEY_LEN: i32 = 66;

#[derive(Clone, Encode, Decode, PassByInner, MaxEncodedLen)]
pub struct Public(pub [u8; COMPRESSED_PUBKEY_LEN]);
...
impl Public {
    /// A new instance from the given 66-byte `data`.
    ///
    /// NOTE: No checking goes on to ensure this is a real public key. Only use it if
    /// you are certain that the array actually is a pubkey.
    pub fn from_raw(data: [u8; COMPRESSED_PUBKEY_LEN]) -> Self {
        Self(data)
    }
...
impl TraitPublic for Public {
    /// A new instance from the given slice that should be 66 bytes long.
    ///
    /// NOTE: No checking goes on to ensure this is a real public key. Only use it if
    /// you are certain that the array actually is a pubkey.
    fn from_slice(data: &[u8]) -> Self {
        let mut r = [0u8; COMPRESSED_PUBKEY_LEN];
...
impl Pair {
    ...
    pub fn sign_prehashed(&self, message: &[u8; 64]) -> Signature {
...
//     use crate::{
//         crypto::{set_default_ss58_version, DEV_PHRASE},
//         keccak_256,
//     };
//     use hex_literal::hex;
//     use serde_json;
