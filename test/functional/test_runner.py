...
`test/functional/test_framework/test_framework.py:MintlayerTestFramework.main`.
...
def main():
    ...
    # TODO: consider using a generated config file
    ...
    config.set("components", "ENABLE_NODE", "true")
    config.add_section("environment")
    script_dir = os.path.dirname(os.path.realpath(__file__))
    src_dir = os.path.join(script_dir, "..", "..")
    build_dir = os.path.join(src_dir, "target", "debug")
    exe_extention = ""
    config.set("environment", "SRCDIR", src_dir)
    config.set("environment", "BUILDDIR", build_dir)
    config.set("environment", "EXEEXT", "")
    ...
    tmpdir = "%s/mintlayer_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)

    logging.debug("Temporary test directory at %s" % tmpdir)

    enable_node = config["components"].getboolean("ENABLE_NODE")
    ...
    if not enable_node:
        print("No functional tests to run. Node must be at least enabled")
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie nodes, we can apply a
