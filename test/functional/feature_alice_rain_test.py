...
class ExampleTest(MintlayerTestFramework):
    ...
    def set_test_params(self):
        ...
        self.extra_args = [["--validator=false"], [], []]
    ...
    def run_test(self):
        ...
        rain = Keypair.create_from_uri('//Rain')
        rude = Keypair.create_from_uri('//Rude')
        ...
        tx1 = utxo.Transaction(
            ...
            outputs=[
                utxo.Output(
                    value=50 * COIN,
                    destination=utxo.DestPubkey(rain.public_key),
                    data=None
                ),
                utxo.Output(
                    value=39999999949 * COIN,
        ...
        tx2 = utxo.Transaction(
            ...
            outputs=[
                utxo.Output(
                    value=30 * COIN,
                    destination=utxo.DestPubkey(rude.public_key),
                    data=None
                ),
                utxo.Output(
                    value=19 * COIN,
        ...
        ).sign(rain, [tx1.outputs[0]])
        client.submit(rain, tx2)
