...
def wait_until(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None):
    ...
    assert_greater_than(timeout - curr_time, time.time() - curr_time)
...
def connect_nodes(from_connection, to_connection):
    to_node_num = to_connection.index
    from_node_num = from_connection.index
    to_peerId = to_connection.peer_id
    from_peerId = from_connection.peer_id

    to_port = p2p_port(to_node_num)
    from_port = p2p_port(from_node_num)

    from_connection.\
        rpc_client.\
        substrate.\
        rpc_request("system_addReservedPeer",
                    ["/ip4/127.0.0.1/tcp/{}/p2p/{}".
                    format(to_port, to_peerId)])
    to_connection.\
        rpc_client.\
        substrate.\
        rpc_request("system_addReservedPeer",
                    ["/ip4/127.0.0.1/tcp/{}/p2p/{}".
                    format(from_port, from_peerId)])

    # make sure we're connected
    def FindPeerInConn(conn, id_to_find):
        res = conn. \
            rpc_client. \
            substrate. \
            rpc_request("system_peers", [])
        for node_data in res["result"]:
            if node_data["peerId"] == id_to_find:
                return True
        return False

    # make sure both nodes see each other
    wait_until(lambda: FindPeerInConn(from_connection, to_peerId))
    wait_until(lambda: FindPeerInConn(to_connection, from_peerId))
